...
function docReady(){
	...
	//datepicker
	$('.datepicker').datepicker({dateFormat: 'yy-mm-dd 00:00'});
	...
	*/
	...
	// our own codes
	// 图表时间区间搜索
	$('.btn-chart-search').click(function(e){
		e.preventDefault();
		//记录下当前正在操作的图表
		var chart_div = $(this).parent().parent().parent().find('div.flotchart')[0];
		var id = $(chart_div).attr("id");
		$('#hide_chartid').val(id);

		var chart_type = $(chart_div).attr("chart_type");
		//饼图或柱状图
		if(chart_type=='2'|| chart_type=='3'){
			alert("暂不支持饼图和柱形图");
			return;	
		}
		$('#watch_div').modal('show');
	});
	// 查看此时间区间内精确的监控数据
	$('.btn-precise-data').click(function(e){
		e.preventDefault();
		var chart_div = $(this).parent().parent().parent().find('div.flotchart')[0];
		var url = get_precise_data_url(chart_div);
		window.open(url);
	});
