    ...
    RGraph.Scatter.prototype.DrawLabels = function ()
    {
        ...
        */
		...
		var xPos  = this.Get('chart.yaxispos') == 'left' ? this.gutterLeft-5 : this.canvas.width - this.gutterRight + 5;
			...
			//原始代码中只允许 chart.ylabels.count  为 1/3/5/10
                        //alert('[SCATTER SCALE] Number of Y labels can be 1/3/5/10 only');
			var interval = (this.grapharea / numYLabels) / 2;
                        if (invert) {
                            for (var i=numYLabels; i>=0; --i) {
                                RGraph.Text(context, font, text_size, xPos,this.gutterTop + ((this.canvas.height - this.gutterTop - this.gutterBottom) * ((numYLabels-i)/numYLabels) ),RGraph.number_format(this,(this.max - (this.max * (i/numYLabels))).toFixed((this.Get('chart.scale.decimals'))), units_pre, units_post),'center', align, boxed);
                            }
                        } else {
                            // numYLabels Y labels
                            for (var i=0; i<numYLabels; ++i) {
    
                                RGraph.Text(context, font, text_size, xPos,this.gutterTop + ((this.canvas.height - this.gutterTop - this.gutterBottom) * (i/numYLabels) ),RGraph.number_format(this, (this.max - ((this.max - this.min) * (i/numYLabels))).toFixed((this.Get('chart.scale.decimals'))), units_pre, units_post),'center', align, boxed);
                            }
			}
